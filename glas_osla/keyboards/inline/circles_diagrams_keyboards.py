...
async def time_keyboard(callback: str):
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.add(InlineKeyboardButton('1️⃣ День', callback_data=f'cd_show_{callback}_d'))
    keyboard.add(InlineKeyboardButton('7️⃣ Неделя', callback_data=f'cd_show_{callback}_w'))
