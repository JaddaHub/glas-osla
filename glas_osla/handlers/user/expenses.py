...
from glas_osla.db.db_commands import get_expenses_category_name, new_expenses_category_name, \
    delete_user_expenses_category, get_user_expenses_subcategories, new_expenses_sub_category_name, \
    delete_user_sub_category, get_user_expenses_category_id_from_sub_category, get_sub_category_name
...
async def get_categories(callback: types.CallbackQuery):
    expenses_categories_keyboard = await keyboards.expenses_categories_keyboard(callback.from_user.id)
...
async def get_sub_categories(callback: types.CallbackQuery):
    ...
    expenses_sub_categories_keyboard = await keyboards.expenses_subcategories_keyboard(callback.from_user.id,
                                                                                       current_category_id)
...
async def change_sub_category(callback: types.CallbackQuery):
    ...
    current_sub_category_name = await get_sub_category_name(current_sub_category_id)
...
async def delete_sub_category(callback: types.CallbackQuery):
    current_sub_category_id = int(callback.data.split('_')[-1])
    await delete_user_sub_category(current_sub_category_id)
    edit_sub_category_keyboard = await keyboards.edit_expenses_sub_category()
...
async def return_from_changing_sub_category(callback: types.CallbackQuery):
    current_sub_category_id = int(callback.data.split('_')[-1])
    current_category_id = await get_user_expenses_category_id_from_sub_category(current_sub_category_id)
    current_category_name = await get_expenses_category_name(current_category_id)
    expenses_sub_categories_keyboard = await keyboards.expenses_subcategories_keyboard(callback.from_user.id,
                                                                                       current_category_id)
    await callback.message.edit_text(f'Подкатегории у {current_category_name}',
...
def setup_expenses_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(return_from_changing_sub_category, ClientFilter(True), text='back_to_c_')
