...
from glas_osla.keyboards.inline import revenues_keyboards as re_keyboards, general_keyboards as gen_keyboards
from glas_osla.states.RevenuesStates import ChangeRevenuesCategoryStates, ChangeRevenuesSubCategoryStates, \
...
async def get_categories(callback: types.CallbackQuery):
    expenses_categories_keyboard = await re_keyboards.revenues_categories_keyboard(callback.from_user.id)
    await callback.message.edit_text(f'{your_revenues_cat_text}', reply_markup=expenses_categories_keyboard)
...
async def get_sub_categories(callback: types.CallbackQuery):
    ...
    expenses_sub_categories_keyboard = await re_keyboards.revenues_subcategories_keyboard(callback.from_user.id,
                                                                                          current_category_id)
...
async def get_new_category_name(message: types.Message, state: FSMContext):
    ...
    await db_commands.new_category_name(message.from_user.id, current_category_id, new_name, RevenueCategory)
...
async def change_sub_category(callback: types.CallbackQuery):
    ...
    current_sub_category_name = await db_commands.get_sub_category_name(current_sub_category_id, RevenueSubCategory)
...
async def get_new_sub_category_name(message: types.Message, state: FSMContext):
    ...
    await db_commands.new_sub_category_name(message.from_user.id, current_sub_category_id, new_name, RevenueSubCategory)
...
async def return_from_changing_category(callback: types.CallbackQuery):
    expenses_categories_keyboard = await re_keyboards.revenues_categories_keyboard(callback.from_user.id)
    await callback.message.edit_text(your_revenues_cat_text, reply_markup=expenses_categories_keyboard)


async def return_from_changing_sub_category(callback: types.CallbackQuery):
    current_sub_category_id = int(callback.data.split('_')[-1])
    current_category_id = await db_commands.get_user_category_id_from_sub_category(current_sub_category_id,
                                                                                   RevenueSubCategory)
    current_category_name = await db_commands.get_category_name(current_category_id, RevenueCategory)
    expenses_sub_categories_keyboard = await re_keyboards.revenues_subcategories_keyboard(callback.from_user.id,
                                                                                          current_category_id)
...
def setup_revenues_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(get_sub_categories, ClientFilter(True), filters.Text(startswith='r_c_'))
    dp.register_callback_query_handler(change_category, ClientFilter(True), filters.Text(startswith='change_r_c_'))
    dp.register_callback_query_handler(edit_category, ClientFilter(True), filters.Text(startswith='edit_r_c_'))
    dp.register_message_handler(get_new_category_name, ClientFilter(True),
                                state=ChangeRevenuesCategoryStates.new_category_name)
    dp.register_callback_query_handler(delete_category, ClientFilter(True), filters.Text(startswith='del_r_c_'))

    dp.register_callback_query_handler(change_sub_category, ClientFilter(True), filters.Text(startswith='r_s_c_'))
    dp.register_callback_query_handler(edit_sub_category, ClientFilter(True), filters.Text(startswith='edit_r_s_c_'))
    ...
    dp.register_callback_query_handler(delete_sub_category, ClientFilter(True), filters.Text(startswith='del_r_s_c_'))
    ...
    dp.register_callback_query_handler(return_from_editing, ClientFilter(True), text='edit_back_to_r_categories')
    ...
    dp.register_callback_query_handler(add_new_sub_category, ClientFilter(True), filters.Text(startswith='add_r_s_c_'))
